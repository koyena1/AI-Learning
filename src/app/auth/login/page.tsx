...
export default function LoginPage() {
  return (
    <main className="relative min-h-screen overflow-hidden bg-white">
      ...
      <div className="absolute left-28 top-24 grid grid-cols-4 gap-3 opacity-30">
        {Array.from({ length: 16 }).map((_, i) => (
          <div key={i} className="h-1.5 w-1.5 rounded-full bg-green-500" />
        ))}
      </div>

      <div className="absolute right-32 bottom-24 grid grid-cols-4 gap-3 opacity-30">
        {Array.from({ length: 16 }).map((_, i) => (
          <div key={i} className="h-1.5 w-1.5 rounded-full bg-green-500" />
        ))}
      </div>
          ...
          <div className="rounded-3xl border border-green-100 bg-white/90 p-10 shadow-2xl backdrop-blur">
            ...
            <div className="mb-8 flex flex-col items-center">
              ...
              <p className="mt-3 text-center text-gray-500">
                Access your dashboard and learning vault.
              </p>
